...
-- Keep track of localPlayer’s character updates
...
local function onRankAdded(character)
    ...
    if not player or player == rankLocalPlayer then return end
    ...
    textLabel.Text = getRank(player.UserId)
    billboardGui.Parent = head

    -- Visibility update with safe checks
    rankRunService.RenderStepped:Connect(function()
        if not rankLocalCharacter then return end
        ...
        textLabel.Visible = (distance <= 30 and onScreen)
    end)
end
